...
bool ShipyardPanel::HasItem(const string &name) const
{
	...
	return shipyard.Has(ship) || player.DepreciatedShipSales(ship) > 0;
}
...
void ShipyardPanel::DrawItem(const string &name, const Point &point)
{
	...
	if(!shipyard.Has(ship) && player.DepreciatedShipSales(ship) > 0)
	{
		const Font &font = FontSet::Get(14);
		const Color &bright = *GameData::Colors().Get("bright");
		string message = "in stock: " + to_string(player.DepreciatedShipSales(ship));
